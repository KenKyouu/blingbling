<?php
require __DIR__ . '/parts/connect_db.php';
$pageName = 'product_details'; // 頁面名稱，可以自定義

if (!isset($_GET['sid'])) {
  header('Location: product-list.php');
  exit;
}
$sid =  intval($_GET['sid']);
$sql = "SELECT *  FROM product WHERE sid=$sid";
$r = $pdo->query($sql)->fetch();

// 如果沒有拿到資料
if (empty($r)) {
  header('Location: product-list.php');
  exit;
}

$product = $pdo->query("SELECT * FROM product WHERE sid=$sid")->fetchAll();
$productclass = $product[0]['class_sid'];
$brandsid = $product[0]['brand_sid'];
$brand = $pdo->query("SELECT `name` FROM brand WHERE sid=$brandsid")->fetchAll();
$brandname = $brand[0]['name'];
$secondclass = $pdo->query("SELECT * FROM `class` WHERE sid=$productclass")->fetchAll();
$firstclasssid = $secondclass[0]['parent'];
$firstclass = $pdo->query("SELECT * FROM `class` WHERE sid=$firstclasssid")->fetchAll();
?>

<?php include __DIR__ . '/parts/html-head.php'; ?>
<link rel="stylesheet" href="./styles/product-details.css">
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.2.0/css/all.min.css">
<?php include __DIR__ . '/parts/navbar.php'; ?>
<section class="content">
  <div class="containerr">
    <div class="product-info1">
      <div class="product-right order-md-2">
        <div class="product-pic">
          <div class="slider-bar">
            <!-- div product-pic可視窗設定  slider-bar滑動用 slider-row排版用-->
            <div class="slider-row">
              <img src="./images/products/<?= $sid ?>_1.png" alt="">
              <img src="./images/products/<?= $sid ?>_2.png" alt="">
              <img src="./images/products/<?= $sid ?>_3.png" alt="">
              <img src="./images/products/<?= $sid ?>_4.png" alt="">
              <!-- <div class="pic_size"><img src="./images/products/<?= $sid ?>_1.png" alt=""></div>
              <div class="pic_size"><img src="./images/products/<?= $sid ?>_2.png" alt=""></div>
              <div class="pic_size"><img src="./images/products/<?= $sid ?>_3.png" alt=""></div>
              <div class="pic_size"><img src="./images/products/<?= $sid ?>_4.png" alt=""></div> -->
            </div>
          </div>
        </div>
      </div>
      <div class="product-left  order-md-1">
        <div class="product-nav">
          <a href="./product-list2.php?cate=<?= $firstclass[0]['sid'] ?>"><?= $firstclass[0]['name'] ?></a>
          <svg width="8" height="20" viewBox="0 0 12 20" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M11.4978 8.89704L2.66869 0.44262C2.52368 0.302369 2.35115 0.191049 2.16106 0.115081C1.97097 0.0391125 1.76708 0 1.56115 0C1.35522 0 1.15133 0.0391125 0.961244 0.115081C0.771154 0.191049 0.598626 0.302369 0.453612 0.44262C0.163076 0.722981 0 1.10224 0 1.49755C0 1.89287 0.163076 2.27212 0.453612 2.55248L8.17519 10.0343L0.453612 17.4412C0.163076 17.7216 0 18.1008 0 18.4962C0 18.8915 0.163076 19.2707 0.453612 19.5511C0.598083 19.6925 0.770368 19.805 0.960483 19.882C1.1506 19.9591 1.35476 19.9992 1.56115 20C1.76754 19.9992 1.97171 19.9591 2.16182 19.882C2.35194 19.805 2.52422 19.6925 2.66869 19.5511L11.4978 11.0967C11.6562 10.9566 11.7825 10.7865 11.8689 10.5972C11.9554 10.4079 12 10.2035 12 9.99686C12 9.79021 11.9554 9.5858 11.8689 9.39651C11.7825 9.20722 11.6562 9.03716 11.4978 8.89704Z" fill="#2d827d" />
          </svg>
          <a href="./product-list2.php?cate=<?= $secondclass[0]['sid'] ?>"><?= $secondclass[0]['name'] ?></a>
          <!-- <svg width="8" height="20" viewBox="0 0 12 20" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M11.4978 8.89704L2.66869 0.44262C2.52368 0.302369 2.35115 0.191049 2.16106 0.115081C1.97097 0.0391125 1.76708 0 1.56115 0C1.35522 0 1.15133 0.0391125 0.961244 0.115081C0.771154 0.191049 0.598626 0.302369 0.453612 0.44262C0.163076 0.722981 0 1.10224 0 1.49755C0 1.89287 0.163076 2.27212 0.453612 2.55248L8.17519 10.0343L0.453612 17.4412C0.163076 17.7216 0 18.1008 0 18.4962C0 18.8915 0.163076 19.2707 0.453612 19.5511C0.598083 19.6925 0.770368 19.805 0.960483 19.882C1.1506 19.9591 1.35476 19.9992 1.56115 20C1.76754 19.9992 1.97171 19.9591 2.16182 19.882C2.35194 19.805 2.52422 19.6925 2.66869 19.5511L11.4978 11.0967C11.6562 10.9566 11.7825 10.7865 11.8689 10.5972C11.9554 10.4079 12 10.2035 12 9.99686C12 9.79021 11.9554 9.5858 11.8689 9.39651C11.7825 9.20722 11.6562 9.03716 11.4978 8.89704Z" fill="#2d827d" />
          </svg>
          <a href="">寵物床/外出籠</a> -->
        </div>

        <div class="product-text">
          <p class="brandname"><?= $brandname ?></p>
          <p class="productname"><?= $product[0]['name'] ?></p>
          <p class="star-icon">
            <svg width="20" height="20" viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M13.1267 5.00989C13.7746 3.2772 16.2254 3.27719 16.8733 5.00989L18.2394 8.66336C18.5205 9.41506 19.2237 9.92594 20.0255 9.96098L23.9223 10.1313C25.7704 10.212 26.5277 12.5429 25.08 13.6945L22.0275 16.1228C21.3995 16.6224 21.1309 17.449 21.3453 18.2223L22.3876 21.9811C22.8819 23.7637 20.8991 25.2042 19.3565 24.1833L16.1038 22.0305C15.4346 21.5876 14.5654 21.5876 13.8962 22.0305L10.6435 24.1833C9.1009 25.2042 7.11812 23.7637 7.61241 21.9811L8.65465 18.2223C8.86909 17.449 8.6005 16.6224 7.97246 16.1228L4.91995 13.6945C3.47227 12.5429 4.22963 10.212 6.07773 10.1313L9.97454 9.96098C10.7763 9.92594 11.4795 9.41506 11.7606 8.66336L13.1267 5.00989Z" fill="#e1a77e" />
            </svg>
            <svg width="20" height="20" viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M13.1267 5.00989C13.7746 3.2772 16.2254 3.27719 16.8733 5.00989L18.2394 8.66336C18.5205 9.41506 19.2237 9.92594 20.0255 9.96098L23.9223 10.1313C25.7704 10.212 26.5277 12.5429 25.08 13.6945L22.0275 16.1228C21.3995 16.6224 21.1309 17.449 21.3453 18.2223L22.3876 21.9811C22.8819 23.7637 20.8991 25.2042 19.3565 24.1833L16.1038 22.0305C15.4346 21.5876 14.5654 21.5876 13.8962 22.0305L10.6435 24.1833C9.1009 25.2042 7.11812 23.7637 7.61241 21.9811L8.65465 18.2223C8.86909 17.449 8.6005 16.6224 7.97246 16.1228L4.91995 13.6945C3.47227 12.5429 4.22963 10.212 6.07773 10.1313L9.97454 9.96098C10.7763 9.92594 11.4795 9.41506 11.7606 8.66336L13.1267 5.00989Z" fill="#e1a77e" />
            </svg>
            <svg width="20" height="20" viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M13.1267 5.00989C13.7746 3.2772 16.2254 3.27719 16.8733 5.00989L18.2394 8.66336C18.5205 9.41506 19.2237 9.92594 20.0255 9.96098L23.9223 10.1313C25.7704 10.212 26.5277 12.5429 25.08 13.6945L22.0275 16.1228C21.3995 16.6224 21.1309 17.449 21.3453 18.2223L22.3876 21.9811C22.8819 23.7637 20.8991 25.2042 19.3565 24.1833L16.1038 22.0305C15.4346 21.5876 14.5654 21.5876 13.8962 22.0305L10.6435 24.1833C9.1009 25.2042 7.11812 23.7637 7.61241 21.9811L8.65465 18.2223C8.86909 17.449 8.6005 16.6224 7.97246 16.1228L4.91995 13.6945C3.47227 12.5429 4.22963 10.212 6.07773 10.1313L9.97454 9.96098C10.7763 9.92594 11.4795 9.41506 11.7606 8.66336L13.1267 5.00989Z" fill="#e1a77e" />
            </svg>
            <svg width="20" height="20" viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M13.1267 5.00989C13.7746 3.2772 16.2254 3.27719 16.8733 5.00989L18.2394 8.66336C18.5205 9.41506 19.2237 9.92594 20.0255 9.96098L23.9223 10.1313C25.7704 10.212 26.5277 12.5429 25.08 13.6945L22.0275 16.1228C21.3995 16.6224 21.1309 17.449 21.3453 18.2223L22.3876 21.9811C22.8819 23.7637 20.8991 25.2042 19.3565 24.1833L16.1038 22.0305C15.4346 21.5876 14.5654 21.5876 13.8962 22.0305L10.6435 24.1833C9.1009 25.2042 7.11812 23.7637 7.61241 21.9811L8.65465 18.2223C8.86909 17.449 8.6005 16.6224 7.97246 16.1228L4.91995 13.6945C3.47227 12.5429 4.22963 10.212 6.07773 10.1313L9.97454 9.96098C10.7763 9.92594 11.4795 9.41506 11.7606 8.66336L13.1267 5.00989Z" fill="#e1a77e" />
            </svg>
            <svg width="20" height="20" viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M13.1267 5.00989C13.7746 3.2772 16.2254 3.27719 16.8733 5.00989L18.2394 8.66336C18.5205 9.41506 19.2237 9.92594 20.0255 9.96098L23.9223 10.1313C25.7704 10.212 26.5277 12.5429 25.08 13.6945L22.0275 16.1228C21.3995 16.6224 21.1309 17.449 21.3453 18.2223L22.3876 21.9811C22.8819 23.7637 20.8991 25.2042 19.3565 24.1833L16.1038 22.0305C15.4346 21.5876 14.5654 21.5876 13.8962 22.0305L10.6435 24.1833C9.1009 25.2042 7.11812 23.7637 7.61241 21.9811L8.65465 18.2223C8.86909 17.449 8.6005 16.6224 7.97246 16.1228L4.91995 13.6945C3.47227 12.5429 4.22963 10.212 6.07773 10.1313L9.97454 9.96098C10.7763 9.92594 11.4795 9.41506 11.7606 8.66336L13.1267 5.00989Z" fill="#c2c2c2" />
            </svg>
          </p>
          <div class="productprice">
            <span>$</span><?= $product[0]['price'] ?>
          </div>
          <div class="pc-price">
            <div class="productprice">
              <span>$</span><?= $product[0]['price'] ?>
            </div>
            <!-- <div class="pc-select-group">
              <div class="product-select">
                <p class="select-title">規格</p>
                <select class="select-info">
                  <option>白色</option>
                  <option>黑色</option>
                  <option>咖啡色</option>
                </select>
              </div>
              <div class="product-select">
                <p class="select-title">尺寸</p>
                <select class="select-info">
                  <option>S</option>
                  <option>M</option>
                  <option>L</option>
                </select>
              </div>
            </div>
          </div>
          -->
          </div>

          <div class="product-info">
            <!-- <div class="product-info-title">
            <p class="describe">商品描述</p>
            <p class="specification">商品規格</p>
          </div> -->
            <div class="long-line">
              <div class="short-line"></div>
            </div>
            <div class="product-info-text">
              <p class="describe-text">
                <?= $product[0]['intro'] ?>
              </p>
              <p class="specification-text"> </p>
            </div>
          </div>

          <div class="pc-add-cart">
            <div class="quantity">
              <button class="quantity-reduce" onclick="numReduce()"><svg width="11" height="4" viewBox="0 0 18 4" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <rect width="18" height="4" rx="1" fill="#2d827d" />
                </svg>
              </button>
              <input class="quantity-num" value=1 id="addtext"></input>
              <button class="quantity-add" onclick="numAdd()"><svg width="11" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path fill-rule="evenodd" clip-rule="evenodd" d="M7.2 17C7.2 17.5523 7.64772 18 8.2 18H9.8C10.3523 18 10.8 17.5523 10.8 17V10.8H17C17.5523 10.8 18 10.3523 18 9.8V8.2C18 7.64772 17.5523 7.2 17 7.2H10.8V1C10.8 0.447715 10.3523 0 9.8 0H8.2C7.64772 0 7.2 0.447716 7.2 1V7.2H1C0.447715 7.2 0 7.64772 0 8.2V9.8C0 10.3523 0.447716 10.8 1 10.8H7.2V17Z" fill="#ffffff" />
                </svg>
              </button>
            </div>
            <div class="cartbtn">
              <button class="add-cartbtn d-flex" data-sid="<?= $sid ?>" onclick="addToCart(event)">
                <p>加入購物車</p>
                <div class="cart">
                  <svg width="18" height="18" viewBox="0 0 26 23" fill="#ffffff" xmlns="http://www.w3.org/2000/svg">
                    <path d="M4.88232 15.6203H20.3076V5.69214H1.72564C1.2217 5.69214 0.871237 6.1319 1.04519 6.54611L4.88232 15.6203Z" fill="" stroke="#ffffff" stroke-miterlimit="10" />
                    <path d="M25.4566 0.546045L25.4564 0.545871L25.454 0.543485C25.454 0.543467 25.454 0.543448 25.454 0.543429C25.396 0.485505 25.3006 0.485524 25.2426 0.543485L19.9379 5.84555L20.1516 6.05907L25.4566 0.756655C25.5145 0.698812 25.5145 0.603888 25.4566 0.546045Z" fill="#ffffff" stroke="#ffffff" />
                    <path d="M18.0269 20.701C18.0269 21.2252 18.4521 21.6507 18.9773 21.6507C19.5025 21.6507 19.9277 21.2252 19.9277 20.701C19.9277 20.1767 19.5024 19.7512 18.9773 19.7512C18.4522 19.7512 18.0269 20.1767 18.0269 20.701Z" fill="#ffffff" stroke="#ffffff" />
                    <path d="M5.38233 20.701C5.38233 21.2252 5.80761 21.6507 6.33276 21.6507C6.85792 21.6507 7.2832 21.2252 7.2832 20.701C7.2832 20.1767 6.8579 19.7512 6.33276 19.7512C5.80762 19.7512 5.38233 20.1767 5.38233 20.701Z" fill="#ffffff" stroke="#ffffff" />
                    <path d="M5.51918 17.9509H19.7908C19.8666 17.9509 19.9277 17.8895 19.9277 17.8143V17.8117C19.9277 17.7365 19.8666 17.675 19.7908 17.675H5.51918C5.44338 17.675 5.38221 17.7365 5.38221 17.8117V17.8143C5.38221 17.8907 5.44207 17.9509 5.51918 17.9509Z" fill="#ffffff" stroke="#ffffff" />
                    <polyline points="15 13.5 17 15.5 22 10.5" stroke-width="1.5"></polyline>
                  </svg>
                </div>
              </button>

            </div>
            <!-- <<<<<<< HEAD
          <div class="heart-icon">
            <svg width="25" height="25" viewBox="0 0 26 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M24.9375 6.30948L24.9317 6.25122L24.9192 6.19403C24.2754 3.2556 21.6922 1.0383 18.5737 1.0383C16.253 1.0383 14.2267 2.26615 13.0775 4.10766C11.9237 2.24427 9.87917 1 7.53714 1C3.92217 1 1.02082 3.96267 1.00002 7.58933H1V7.59507V7.59512V7.59518V7.59523V7.59528V7.59533V7.59539V7.59544V7.59549V7.59555V7.5956V7.59565V7.59571V7.59576V7.59581V7.59587V7.59592V7.59597V7.59603V7.59608V7.59614V7.59619V7.59624V7.5963V7.59635V7.59641V7.59646V7.59651V7.59657V7.59662V7.59668V7.59673V7.59678V7.59684V7.59689V7.59695V7.597V7.59706V7.59711V7.59717V7.59722V7.59728V7.59733V7.59738V7.59744V7.59749V7.59755V7.5976V7.59766V7.59771V7.59777V7.59783V7.59788V7.59794V7.59799V7.59805V7.5981V7.59816V7.59821V7.59827V7.59832V7.59838V7.59843V7.59849V7.59855V7.5986V7.59866V7.59871V7.59877V7.59882V7.59888V7.59894V7.59899V7.59905V7.5991V7.59916V7.59922V7.59927V7.59933V7.59938V7.59944V7.5995V7.59955V7.59961V7.59967V7.59972V7.59978V7.59984V7.59989V7.59995V7.60001V7.60006V7.60012V7.60018V7.60023V7.60029V7.60035V7.6004V7.60046V7.60052V7.60057V7.60063V7.60069V7.60074V7.6008V7.60086V7.60092V7.60097V7.60103V7.60109V7.60114V7.6012V7.60126V7.60132V7.60137V7.60143V7.60149V7.60155V7.6016V7.60166V7.60172V7.60178V7.60183V7.60189V7.60195V7.60201V7.60206V7.60212V7.60218V7.60224V7.60229V7.60235V7.60241V7.60247V7.60253V7.60258V7.60264V7.6027V7.60276V7.60282V7.60287V7.60293V7.60299V7.60305V7.60311V7.60316V7.60322V7.60328V7.60334V7.6034V7.60346V7.60351V7.60357V7.60363V7.60369V7.60375V7.60381V7.60386V7.60392V7.60398V7.60404V7.6041V7.60416V7.60421V7.60427V7.60433V7.60439V7.60445V7.60451V7.60457V7.60462V7.60468V7.60474V7.6048V7.60486V7.60492V7.60498V7.60504V7.60509V7.60515V7.60521V7.60527V7.60533V7.60539V7.60545V7.60551V7.60557V7.60562V7.60568V7.60574V7.6058V7.60586V7.60592V7.60598V7.60604V7.6061V7.60615V7.60621V7.60627V7.60633V7.60639V7.60645V7.60651V7.60657V7.60663V7.60669V7.60675V7.6068V7.60686V7.60692V7.60698V7.60704V7.6071V7.60716V7.60722V7.60728V7.60734V7.6074V7.60746V7.60752V7.60757V7.60763V7.60769V7.60775V7.60781V7.60787V7.60793V7.60799V7.60805V7.60811V7.60817V7.60823V7.60829V7.60835V7.60841V7.60846V7.60852V7.60858V7.60864V7.6087V7.60876V7.60882V7.60888V7.60894V7.609V7.60906V7.60912V7.60918V7.60924V7.6093V7.60935V7.60941V7.60947V7.60953V7.60959V7.60965V7.60971V7.60977V7.60983V7.60989V7.60995V7.61001V7.61007V7.61013V7.61019V7.61025V7.61031V7.61036V7.61042V7.61048V7.61054V7.6106V7.61066V7.61072V7.61078V7.61084V7.6109V7.61096V7.61102V7.61108V7.61114V7.6112V7.61126V7.61131V7.61137V7.61143V7.61149V7.61155V7.61161V7.61167V7.61173V7.61179V7.61185V7.61191V7.61197V7.61203V7.61208V7.61214V7.6122V7.61226V7.61232V7.61238V7.61244V7.6125V7.61256V7.61262V7.61268V7.61274V7.6128V7.61285V7.61291V7.61297V7.61303V7.61309V7.61315V7.61321V7.61327V7.61333V7.61339V7.61344V7.6135V7.61356V7.61362V7.61368V7.61374V7.6138V7.61386V7.61392V7.61398V7.61403V7.61409V7.61415V7.61421V7.61427V7.61433V7.61439V7.61445V7.6145V7.61456V7.61462V7.61468V7.61474V7.6148V7.61486V7.61492V7.61497V7.61503V7.61509V7.61515V7.61521V7.61527V7.61532V7.61538V7.61544V7.6155V7.61556V7.61562V7.61568V7.61573V7.61579V7.61585V7.61591V7.61597V7.61603V7.61608V7.61614V7.6162V7.61626V7.61632V7.61637V7.61643V7.61649V7.61655V7.61661V7.61666V7.61672V7.61678V7.61684V7.6169V7.61695V7.61701V7.61707V7.61713V7.61719V7.61724V7.6173V7.61736V7.61742V7.61747V7.61753V7.61759V7.61765V7.6177V7.61776V7.61782V7.61788V7.61793V7.61799V7.61805V7.61811V7.61816V7.61822V7.61828V7.61834V7.61839V7.61845V7.61851V7.61856V7.61862V7.61868V7.61874V7.61879V7.61885V7.61891V7.61896V7.61902V7.61908V7.61913V7.61919V7.61925V7.6193V7.61936V7.61942V7.61947V7.61953V7.61959V7.61964V7.6197V7.61976V7.61981V7.61987V7.61993V7.61998V7.62004V7.62009V7.62015V7.62021V7.62026V7.62032V7.62037V7.62043V7.62049V7.62054V7.6206V7.62065V7.62071V7.62077V7.62082V7.62088V7.62093V7.62099V7.62104V7.6211V7.62116V7.62121V7.62127V7.62132V7.62138V7.62143V7.62149V7.62154V7.6216V7.62165V7.62171V7.62176V7.62182V7.62187V7.62193V7.62198V7.62204V7.62209V7.62215V7.6222V7.62226V7.62231V7.62237V7.62242V7.62248V7.62253V7.62258V7.62264V7.62269V7.62275V7.6228V7.62286V7.62291V7.62296V7.62302V7.62307V7.62313V7.62318V7.62323V7.62329V7.62334V7.62339V7.62345V7.6235V7.62356V7.62361V7.62366V7.62372V7.62377V7.62382V7.62388V7.62393V7.62398V7.62404V7.62409V7.62414V7.62419V7.62425V7.6243V7.62435C1 8.39113 1.13087 9.12654 1.36639 9.81117C2.2291 12.6033 4.50686 15.4809 6.67792 17.7685C8.88167 20.0905 11.1006 21.9321 11.9848 22.6419L11.9867 22.6434C12.5747 23.113 13.4145 23.1239 14.0133 22.6456L14.0145 22.6446C14.8513 21.9741 16.8843 20.2885 18.9681 18.1415C21.0325 16.0144 23.2349 13.3462 24.3136 10.708L24.3136 10.708C24.7323 9.68393 25 8.62233 25 7.57255C25 7.1424 24.9783 6.72207 24.9375 6.30948Z" stroke="#d45a6a" stroke-width="2" />
======= -->
            <div class="heart-icon" data-sid="<?= $sid ?>" onclick="heart(event)">
              <svg width="20" height="20" viewBox="0 0 26 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path class="heart" d="M24.9375 6.30948L24.9317 6.25122L24.9192 6.19403C24.2754 3.2556 21.6922 1.0383 18.5737 1.0383C16.253 1.0383 14.2267 2.26615 13.0775 4.10766C11.9237 2.24427 9.87917 1 7.53714 1C3.92217 1 1.02082 3.96267 1.00002 7.58933H1V7.59507V7.59512V7.59518V7.59523V7.59528V7.59533V7.59539V7.59544V7.59549V7.59555V7.5956V7.59565V7.59571V7.59576V7.59581V7.59587V7.59592V7.59597V7.59603V7.59608V7.59614V7.59619V7.59624V7.5963V7.59635V7.59641V7.59646V7.59651V7.59657V7.59662V7.59668V7.59673V7.59678V7.59684V7.59689V7.59695V7.597V7.59706V7.59711V7.59717V7.59722V7.59728V7.59733V7.59738V7.59744V7.59749V7.59755V7.5976V7.59766V7.59771V7.59777V7.59783V7.59788V7.59794V7.59799V7.59805V7.5981V7.59816V7.59821V7.59827V7.59832V7.59838V7.59843V7.59849V7.59855V7.5986V7.59866V7.59871V7.59877V7.59882V7.59888V7.59894V7.59899V7.59905V7.5991V7.59916V7.59922V7.59927V7.59933V7.59938V7.59944V7.5995V7.59955V7.59961V7.59967V7.59972V7.59978V7.59984V7.59989V7.59995V7.60001V7.60006V7.60012V7.60018V7.60023V7.60029V7.60035V7.6004V7.60046V7.60052V7.60057V7.60063V7.60069V7.60074V7.6008V7.60086V7.60092V7.60097V7.60103V7.60109V7.60114V7.6012V7.60126V7.60132V7.60137V7.60143V7.60149V7.60155V7.6016V7.60166V7.60172V7.60178V7.60183V7.60189V7.60195V7.60201V7.60206V7.60212V7.60218V7.60224V7.60229V7.60235V7.60241V7.60247V7.60253V7.60258V7.60264V7.6027V7.60276V7.60282V7.60287V7.60293V7.60299V7.60305V7.60311V7.60316V7.60322V7.60328V7.60334V7.6034V7.60346V7.60351V7.60357V7.60363V7.60369V7.60375V7.60381V7.60386V7.60392V7.60398V7.60404V7.6041V7.60416V7.60421V7.60427V7.60433V7.60439V7.60445V7.60451V7.60457V7.60462V7.60468V7.60474V7.6048V7.60486V7.60492V7.60498V7.60504V7.60509V7.60515V7.60521V7.60527V7.60533V7.60539V7.60545V7.60551V7.60557V7.60562V7.60568V7.60574V7.6058V7.60586V7.60592V7.60598V7.60604V7.6061V7.60615V7.60621V7.60627V7.60633V7.60639V7.60645V7.60651V7.60657V7.60663V7.60669V7.60675V7.6068V7.60686V7.60692V7.60698V7.60704V7.6071V7.60716V7.60722V7.60728V7.60734V7.6074V7.60746V7.60752V7.60757V7.60763V7.60769V7.60775V7.60781V7.60787V7.60793V7.60799V7.60805V7.60811V7.60817V7.60823V7.60829V7.60835V7.60841V7.60846V7.60852V7.60858V7.60864V7.6087V7.60876V7.60882V7.60888V7.60894V7.609V7.60906V7.60912V7.60918V7.60924V7.6093V7.60935V7.60941V7.60947V7.60953V7.60959V7.60965V7.60971V7.60977V7.60983V7.60989V7.60995V7.61001V7.61007V7.61013V7.61019V7.61025V7.61031V7.61036V7.61042V7.61048V7.61054V7.6106V7.61066V7.61072V7.61078V7.61084V7.6109V7.61096V7.61102V7.61108V7.61114V7.6112V7.61126V7.61131V7.61137V7.61143V7.61149V7.61155V7.61161V7.61167V7.61173V7.61179V7.61185V7.61191V7.61197V7.61203V7.61208V7.61214V7.6122V7.61226V7.61232V7.61238V7.61244V7.6125V7.61256V7.61262V7.61268V7.61274V7.6128V7.61285V7.61291V7.61297V7.61303V7.61309V7.61315V7.61321V7.61327V7.61333V7.61339V7.61344V7.6135V7.61356V7.61362V7.61368V7.61374V7.6138V7.61386V7.61392V7.61398V7.61403V7.61409V7.61415V7.61421V7.61427V7.61433V7.61439V7.61445V7.6145V7.61456V7.61462V7.61468V7.61474V7.6148V7.61486V7.61492V7.61497V7.61503V7.61509V7.61515V7.61521V7.61527V7.61532V7.61538V7.61544V7.6155V7.61556V7.61562V7.61568V7.61573V7.61579V7.61585V7.61591V7.61597V7.61603V7.61608V7.61614V7.6162V7.61626V7.61632V7.61637V7.61643V7.61649V7.61655V7.61661V7.61666V7.61672V7.61678V7.61684V7.6169V7.61695V7.61701V7.61707V7.61713V7.61719V7.61724V7.6173V7.61736V7.61742V7.61747V7.61753V7.61759V7.61765V7.6177V7.61776V7.61782V7.61788V7.61793V7.61799V7.61805V7.61811V7.61816V7.61822V7.61828V7.61834V7.61839V7.61845V7.61851V7.61856V7.61862V7.61868V7.61874V7.61879V7.61885V7.61891V7.61896V7.61902V7.61908V7.61913V7.61919V7.61925V7.6193V7.61936V7.61942V7.61947V7.61953V7.61959V7.61964V7.6197V7.61976V7.61981V7.61987V7.61993V7.61998V7.62004V7.62009V7.62015V7.62021V7.62026V7.62032V7.62037V7.62043V7.62049V7.62054V7.6206V7.62065V7.62071V7.62077V7.62082V7.62088V7.62093V7.62099V7.62104V7.6211V7.62116V7.62121V7.62127V7.62132V7.62138V7.62143V7.62149V7.62154V7.6216V7.62165V7.62171V7.62176V7.62182V7.62187V7.62193V7.62198V7.62204V7.62209V7.62215V7.6222V7.62226V7.62231V7.62237V7.62242V7.62248V7.62253V7.62258V7.62264V7.62269V7.62275V7.6228V7.62286V7.62291V7.62296V7.62302V7.62307V7.62313V7.62318V7.62323V7.62329V7.62334V7.62339V7.62345V7.6235V7.62356V7.62361V7.62366V7.62372V7.62377V7.62382V7.62388V7.62393V7.62398V7.62404V7.62409V7.62414V7.62419V7.62425V7.6243V7.62435C1 8.39113 1.13087 9.12654 1.36639 9.81117C2.2291 12.6033 4.50686 15.4809 6.67792 17.7685C8.88167 20.0905 11.1006 21.9321 11.9848 22.6419L11.9867 22.6434C12.5747 23.113 13.4145 23.1239 14.0133 22.6456L14.0145 22.6446C14.8513 21.9741 16.8843 20.2885 18.9681 18.1415C21.0325 16.0144 23.2349 13.3462 24.3136 10.708L24.3136 10.708C24.7323 9.68393 25 8.62233 25 7.57255C25 7.1424 24.9783 6.72207 24.9375 6.30948Z" stroke="#d45a6a" stroke-width="2" />

              </svg>
            </div>

          </div>
        </div>
      </div>

    </div>
</section>

<section class="content2">
  <div class="mightlike">
    <div class="mightlike-title d-flex flex-column align-items-end">
      <p class="mightlike-title-text">..YOU MIGHT LIKE</p>
      <div class="mightlike-capsule">推薦商品</div>
    </div>
    <div class="product-list">
      <!-- <button type="slick-button" class="slick-prev">
        <i class="fa-solid fa-caret-left"></i>
      </button> -->
      <div class="product-list-scroll">
        <div class="product-list-row">
          <?php
          $childclass = $pdo->query("SELECT * FROM `class` WHERE `parent`=$firstclasssid")->fetchAll();
          $secondclassArr = [];
          foreach ($childclass as $cc) {
            array_push($secondclassArr, $cc['sid']);
          }
          $recommendwhere = " WHERE 1 AND (`class_sid` =" . implode(" OR `class_sid`=", $secondclassArr) . ") ";
          $recommendsql =  "SELECT * FROM `product` $recommendwhere  ORDER BY RAND() LIMIT 6";
          $recommend = $pdo->query($recommendsql)->fetchAll();
          ?>
          <?php foreach ($recommend as $rc) : ?>
            <a class="firsr-card" href="./product_details.php?sid=<?= $rc['sid'] ?>">
              <div class="product-card">
                <div class="product-image" style="background-image: url(./images/products/<?= $rc['sid'] ?>_1.png);">
                  <div class="heart">
                    <svg width="29" height="29" viewBox="0 0 29 29" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <g clip-path="url(#clip0_2075_56017)">
                        <g opacity="0.5">
                          <path opacity="0.06" d="M28.8004 14.5989C28.8004 6.75665 22.4436 0.399902 14.6014 0.399902C6.75921 0.399902 0.399902 6.75665 0.399902 14.5989C0.399902 22.4411 6.75665 28.8004 14.6014 28.8004C22.4462 28.8004 28.8029 22.4436 28.8029 14.5989H28.8004Z" fill="url(#paint0_linear_2075_56017)" />
                          <path opacity="0.12" d="M28.603 14.5987C28.603 6.86659 22.3333 0.596924 14.6012 0.596924C6.86915 0.596924 0.596924 6.86659 0.596924 14.5987C0.596924 22.3307 6.86659 28.6004 14.5987 28.6004C22.3307 28.6004 28.6004 22.3307 28.6004 14.5987H28.603Z" fill="url(#paint1_linear_2075_56017)" />
                          <path opacity="0.19" d="M28.4034 14.5987C28.4034 6.97422 22.2234 0.794189 14.6014 0.794189C6.97946 0.794189 0.796875 6.97678 0.796875 14.5987C0.796875 22.2207 6.9769 28.4033 14.6014 28.4033C22.2259 28.4033 28.4034 22.2232 28.4034 14.5987V14.5987Z" fill="url(#paint2_linear_2075_56017)" />
                          <path opacity="0.25" d="M28.2034 14.5987C28.2034 7.08428 22.113 0.993896 14.5987 0.993896C7.08428 0.993896 0.993896 7.08685 0.993896 14.5987C0.993896 22.1105 7.08428 28.2034 14.5987 28.2034C22.113 28.2034 28.2034 22.113 28.2034 14.5987V14.5987Z" fill="url(#paint3_linear_2075_56017)" />
                          <path opacity="0.31" d="M28.0064 14.5988C28.0064 7.1946 22.0031 1.19385 14.6014 1.19385C7.19972 1.19385 1.19385 7.19716 1.19385 14.5988C1.19385 22.0005 7.19716 28.0038 14.6014 28.0038C22.0057 28.0038 28.0064 22.0005 28.0064 14.5988V14.5988Z" fill="url(#paint4_linear_2075_56017)" />
                          <path opacity="0.38" d="M27.8066 14.5987C27.8066 7.30454 21.8929 1.39087 14.5988 1.39087C7.30466 1.39087 1.39355 7.30454 1.39355 14.5987C1.39355 21.8928 7.30722 27.8064 14.6013 27.8064C21.8955 27.8064 27.8091 21.8928 27.8091 14.5987H27.8066Z" fill="url(#paint5_linear_2075_56017)" />
                          <path opacity="0.44" d="M27.6094 14.5988C27.6094 7.41485 21.7854 1.59082 14.6014 1.59082C7.41741 1.59082 1.59082 7.41485 1.59082 14.5988C1.59082 21.7828 7.41485 27.6069 14.5988 27.6069C21.7828 27.6069 27.6069 21.7828 27.6069 14.5988H27.6094Z" fill="url(#paint6_linear_2075_56017)" />
                          <path opacity="0.5" d="M27.4096 14.5987C27.4096 7.52479 21.6752 1.78784 14.6013 1.78784C7.52748 1.78784 1.79053 7.52479 1.79053 14.5987C1.79053 21.6725 7.52492 27.4095 14.6013 27.4095C21.6778 27.4095 27.4096 21.6751 27.4096 14.5987V14.5987Z" fill="url(#paint7_linear_2075_56017)" />
                          <path opacity="0.56" d="M27.2099 14.5988C27.2099 7.6351 21.5651 1.98779 14.5988 1.98779C7.63254 1.98779 1.98779 7.6351 1.98779 14.5988C1.98779 21.5626 7.6351 27.2099 14.5988 27.2099C21.5626 27.2099 27.2099 21.5626 27.2099 14.5988V14.5988Z" fill="url(#paint8_linear_2075_56017)" />
                          <path opacity="0.62" d="M27.0126 14.5988C27.0126 7.74517 21.4549 2.1875 14.6013 2.1875C7.74773 2.1875 2.1875 7.74517 2.1875 14.5988C2.1875 21.4524 7.74517 27.0101 14.5988 27.0101C21.4524 27.0101 27.0101 21.4524 27.0101 14.5988H27.0126Z" fill="url(#paint9_linear_2075_56017)" />
                          <path opacity="0.69" d="M26.8128 14.5988C26.8128 7.8528 21.3448 2.38477 14.5987 2.38477C7.85268 2.38477 2.38721 7.85536 2.38721 14.5988C2.38721 21.3423 7.85524 26.8129 14.6013 26.8129C21.3473 26.8129 26.8153 21.3449 26.8153 14.5988H26.8128Z" fill="url(#paint10_linear_2075_56017)" />
                          <path opacity="0.75" d="M26.6156 14.5988C26.6156 7.96286 21.2372 2.58447 14.6013 2.58447C7.96542 2.58447 2.58447 7.96542 2.58447 14.5988C2.58447 21.2321 7.96286 26.6131 14.5988 26.6131C21.2347 26.6131 26.6131 21.2347 26.6131 14.5988H26.6156Z" fill="url(#paint11_linear_2075_56017)" />
                          <path opacity="0.81" d="M26.4158 14.5988C26.4158 8.07305 21.1245 2.78174 14.6013 2.78174C8.07805 2.78174 2.78418 8.07305 2.78418 14.5988C2.78418 21.1246 8.07549 26.4159 14.6013 26.4159C21.1271 26.4159 26.4158 21.1246 26.4158 14.5988V14.5988Z" fill="url(#paint12_linear_2075_56017)" />
                          <path opacity="0.88" d="M26.216 14.5988C26.216 8.18312 21.0143 2.98145 14.5987 2.98145C8.183 2.98145 2.98389 8.18312 2.98389 14.5988C2.98389 21.0144 8.18556 26.2161 14.6012 26.2161C21.0169 26.2161 26.2185 21.0144 26.2185 14.5988H26.216Z" fill="url(#paint13_linear_2075_56017)" />
                          <path opacity="0.94" d="M26.0188 14.5987C26.0188 8.29319 20.9068 3.18115 14.6013 3.18115C8.29575 3.18115 3.18115 8.29319 3.18115 14.5987C3.18115 20.9042 8.29318 26.0163 14.5987 26.0163C20.9042 26.0163 26.0163 20.9042 26.0163 14.5987H26.0188Z" fill="url(#paint14_linear_2075_56017)" />
                          <path d="M25.819 14.5988C25.819 8.40337 20.7966 3.37842 14.5986 3.37842C8.40069 3.37842 3.38086 8.40337 3.38086 14.5988C3.38086 20.7942 8.40325 25.8191 14.6012 25.8191C20.7992 25.8191 25.8216 20.7967 25.8216 14.5988H25.819Z" fill="url(#paint15_linear_2075_56017)" />
                        </g>
                        <path d="M24.5053 19.0425C26.9589 13.5719 24.5156 7.14853 19.045 4.69496C13.5744 2.24139 7.15106 4.68471 4.69749 10.1553C2.24392 15.6259 4.68981 22.0492 10.1578 24.5028C15.6284 26.9564 22.0518 24.513 24.5053 19.0425Z" fill="url(#paint16_linear_2075_56017)" />
                        <path d="M25.458 14.5986C25.458 19.6236 22.044 23.8494 17.4109 25.089C16.5145 25.3298 15.572 25.4578 14.6013 25.4578C13.8355 25.4578 13.0902 25.3784 12.3705 25.2273C7.44547 24.2003 3.74463 19.831 3.74463 14.5986C3.74463 8.60298 8.60567 3.74194 14.6013 3.74194C20.5969 3.74194 25.458 8.60298 25.458 14.5986Z" fill="url(#paint17_linear_2075_56017)" />
                        <path d="M14.6474 21.6776L14.6089 21.6674L14.5705 21.6776L14.4886 21.6238L14.4937 21.6161C13.8432 21.1884 7.90132 17.106 7.02797 15.2722C6.02912 13.1772 6.92296 10.6621 9.01797 9.66587C10.0322 9.18181 11.1745 9.1229 12.2322 9.49939C13.2797 9.87076 14.1198 10.6237 14.6064 11.62C15.6155 9.55061 18.1151 8.67471 20.1973 9.66587C21.2116 10.1499 21.9773 10.9977 22.3538 12.058C22.7303 13.1157 22.6714 14.258 22.1873 15.2748C21.3114 17.1137 15.3696 21.1935 14.7216 21.6187L14.7267 21.6264L14.6448 21.6802L14.6474 21.6776Z" fill="#D4CFC6" />
                        <path d="M14.6474 21.6776L14.6089 21.6674L14.5705 21.6776L14.4886 21.6238L14.4937 21.6161C13.8432 21.1884 7.90132 17.106 7.02797 15.2722C6.02912 13.1772 6.92296 10.6621 9.01797 9.66587C10.0322 9.18181 11.1745 9.1229 12.2322 9.49939C13.2797 9.87076 14.1198 10.6237 14.6064 11.62C15.6155 9.55061 18.1151 8.67471 20.1973 9.66587C21.2116 10.1499 21.9773 10.9977 22.3538 12.058C22.7303 13.1157 22.6714 14.258 22.1873 15.2748C21.3114 17.1137 15.3696 21.1935 14.7216 21.6187L14.7267 21.6264L14.6448 21.6802L14.6474 21.6776ZM10.8261 9.67867C10.2704 9.67867 9.71716 9.80161 9.20237 10.0475C8.29061 10.4829 7.60166 11.2461 7.26359 12.1963C6.92552 13.149 6.97931 14.176 7.41214 15.0878C8.19841 16.7372 13.733 20.6173 14.6038 21.191H14.6089H14.6115C15.4848 20.6147 21.0195 16.7346 21.8032 15.0878C22.2386 14.176 22.2898 13.149 21.9517 12.1963C21.6136 11.2435 20.9247 10.4803 20.0129 10.0475C18.1305 9.15108 15.869 9.95271 14.9726 11.8352C14.9137 11.9555 14.8599 12.1118 14.8113 12.2962L14.6064 13.0722L14.4015 12.2962C14.3528 12.1118 14.299 11.9555 14.2401 11.8352C13.8047 10.9234 13.0415 10.2344 12.0888 9.89637C11.6764 9.75038 11.2487 9.67611 10.821 9.67611L10.8261 9.67867Z" fill="#D4CFC6" />
                      </g>
                      <defs>
                        <linearGradient id="paint0_linear_2075_56017" x1="0.492103" y1="13.9073" x2="28.7184" y2="15.2929" gradientUnits="userSpaceOnUse">
                          <stop stop-color="white" />
                          <stop offset="0.27" stop-color="#FBFBFB" />
                          <stop offset="0.54" stop-color="#EFF0F1" />
                          <stop offset="0.8" stop-color="#DCDFE1" />
                          <stop offset="1" stop-color="#C7CCD0" />
                        </linearGradient>
                        <linearGradient id="paint1_linear_2075_56017" x1="0.81206" y1="13.2413" x2="28.4058" y2="15.9586" gradientUnits="userSpaceOnUse">
                          <stop stop-color="white" />
                          <stop offset="0.27" stop-color="#FBFBFB" />
                          <stop offset="0.54" stop-color="#EFF0F1" />
                          <stop offset="0.8" stop-color="#DBDFE1" />
                          <stop offset="1" stop-color="#C7CCD0" />
                        </linearGradient>
                        <linearGradient id="paint2_linear_2075_56017" x1="1.16312" y1="12.6062" x2="28.0628" y2="16.5964" gradientUnits="userSpaceOnUse">
                          <stop stop-color="white" />
                          <stop offset="0.27" stop-color="#FBFBFB" />
                          <stop offset="0.54" stop-color="#EFF0F1" />
                          <stop offset="0.8" stop-color="#DBDEE1" />
                          <stop offset="1" stop-color="#C7CCD0" />
                        </linearGradient>
                        <linearGradient id="paint3_linear_2075_56017" x1="1.53942" y1="12.0017" x2="27.6938" y2="17.2033" gradientUnits="userSpaceOnUse">
                          <stop stop-color="white" />
                          <stop offset="0.27" stop-color="#FBFBFB" />
                          <stop offset="0.54" stop-color="#EFF0F1" />
                          <stop offset="0.8" stop-color="#DBDEE1" />
                          <stop offset="1" stop-color="#C7CCD0" />
                        </linearGradient>
                        <linearGradient id="paint4_linear_2075_56017" x1="1.93914" y1="11.4282" x2="27.3021" y2="17.7823" gradientUnits="userSpaceOnUse">
                          <stop stop-color="white" />
                          <stop offset="0.27" stop-color="#FBFBFB" />
                          <stop offset="0.54" stop-color="#EFF0F1" />
                          <stop offset="0.8" stop-color="#DBDEE1" />
                          <stop offset="1" stop-color="#C7CCD0" />
                        </linearGradient>
                        <linearGradient id="paint5_linear_2075_56017" x1="2.36167" y1="10.8876" x2="26.8846" y2="18.3251" gradientUnits="userSpaceOnUse">
                          <stop stop-color="white" />
                          <stop offset="0.27" stop-color="#FBFBFB" />
                          <stop offset="0.53" stop-color="#EFF0F1" />
                          <stop offset="0.8" stop-color="#DBDEE0" />
                          <stop offset="1" stop-color="#C7CCD0" />
                        </linearGradient>
                        <linearGradient id="paint6_linear_2075_56017" x1="2.8048" y1="10.3806" x2="26.4492" y2="18.8401" gradientUnits="userSpaceOnUse">
                          <stop stop-color="white" />
                          <stop offset="0.27" stop-color="#FBFBFB" />
                          <stop offset="0.53" stop-color="#EFF0F1" />
                          <stop offset="0.8" stop-color="#DBDEE0" />
                          <stop offset="1" stop-color="#C7CCD0" />
                        </linearGradient>
                        <linearGradient id="paint7_linear_2075_56017" x1="3.26574" y1="9.90409" x2="25.9933" y2="19.3188" gradientUnits="userSpaceOnUse">
                          <stop stop-color="white" />
                          <stop offset="0.27" stop-color="#FAFBFB" />
                          <stop offset="0.53" stop-color="#EEF0F1" />
                          <stop offset="0.8" stop-color="#DADEE0" />
                          <stop offset="1" stop-color="#C7CCD0" />
                        </linearGradient>
                        <linearGradient id="paint8_linear_2075_56017" x1="3.74473" y1="9.46632" x2="25.5195" y2="19.7647" gradientUnits="userSpaceOnUse">
                          <stop stop-color="white" />
                          <stop offset="0.27" stop-color="#FAFBFB" />
                          <stop offset="0.53" stop-color="#EEF0F1" />
                          <stop offset="0.8" stop-color="#DADDE0" />
                          <stop offset="1" stop-color="#C7CCD0" />
                        </linearGradient>
                        <linearGradient id="paint9_linear_2075_56017" x1="4.23385" y1="9.05903" x2="25.0329" y2="20.1769" gradientUnits="userSpaceOnUse">
                          <stop stop-color="white" />
                          <stop offset="0.27" stop-color="#FAFBFB" />
                          <stop offset="0.53" stop-color="#EEF0F1" />
                          <stop offset="0.8" stop-color="#DADDE0" />
                          <stop offset="1" stop-color="#C7CCD0" />
                        </linearGradient>
                        <linearGradient id="paint10_linear_2075_56017" x1="4.73577" y1="8.68773" x2="24.5359" y2="20.556" gradientUnits="userSpaceOnUse">
                          <stop stop-color="white" />
                          <stop offset="0.26" stop-color="#FAFBFB" />
                          <stop offset="0.53" stop-color="#EEF0F1" />
                          <stop offset="0.81" stop-color="#DADDE0" />
                          <stop offset="1" stop-color="#C7CCD0" />
                        </linearGradient>
                        <linearGradient id="paint11_linear_2075_56017" x1="5.24806" y1="8.3496" x2="24.0263" y2="20.8992" gradientUnits="userSpaceOnUse">
                          <stop stop-color="white" />
                          <stop offset="0.26" stop-color="#FAFBFB" />
                          <stop offset="0.53" stop-color="#EEF0F1" />
                          <stop offset="0.81" stop-color="#DADDE0" />
                          <stop offset="1" stop-color="#C7CCD0" />
                        </linearGradient>
                        <linearGradient id="paint12_linear_2075_56017" x1="5.76534" y1="8.04744" x2="23.5115" y2="21.2091" gradientUnits="userSpaceOnUse">
                          <stop stop-color="white" />
                          <stop offset="0.26" stop-color="#FAFBFB" />
                          <stop offset="0.53" stop-color="#EEEFF1" />
                          <stop offset="0.81" stop-color="#DADDDF" />
                          <stop offset="1" stop-color="#C7CCD0" />
                        </linearGradient>
                        <linearGradient id="paint13_linear_2075_56017" x1="6.28775" y1="7.77846" x2="22.9889" y2="21.4857" gradientUnits="userSpaceOnUse">
                          <stop stop-color="white" />
                          <stop offset="0.26" stop-color="#FAFBFB" />
                          <stop offset="0.53" stop-color="#EEEFF1" />
                          <stop offset="0.81" stop-color="#D9DDDF" />
                          <stop offset="1" stop-color="#C7CCD0" />
                        </linearGradient>
                        <linearGradient id="paint14_linear_2075_56017" x1="6.81541" y1="7.54277" x2="22.464" y2="21.7264" gradientUnits="userSpaceOnUse">
                          <stop stop-color="white" />
                          <stop offset="0.26" stop-color="#FAFBFB" />
                          <stop offset="0.53" stop-color="#EEEFF1" />
                          <stop offset="0.81" stop-color="#D9DCDF" />
                          <stop offset="1" stop-color="#C7CCD0" />
                        </linearGradient>
                        <linearGradient id="paint15_linear_2075_56017" x1="7.34294" y1="7.34306" x2="21.9363" y2="21.9364" gradientUnits="userSpaceOnUse">
                          <stop stop-color="white" />
                          <stop offset="0.26" stop-color="#FAFBFB" />
                          <stop offset="0.54" stop-color="#EEEFF0" />
                          <stop offset="0.81" stop-color="#D9DCDF" />
                          <stop offset="1" stop-color="#C7CCD0" />
                        </linearGradient>
                        <linearGradient id="paint16_linear_2075_56017" x1="3.74219" y1="14.5989" x2="25.4581" y2="14.5989" gradientUnits="userSpaceOnUse">
                          <stop stop-color="#DEDFE6" />
                          <stop offset="1" stop-color="white" />
                        </linearGradient>
                        <linearGradient id="paint17_linear_2075_56017" x1="3.74207" y1="14.5986" x2="25.458" y2="14.5986" gradientUnits="userSpaceOnUse">
                          <stop stop-color="#DEDFE6" />
                          <stop offset="1" stop-color="white" />
                        </linearGradient>
                        <clipPath id="clip0_2075_56017">
                          <rect width="28.8" height="28.8" fill="white" transform="translate(0.199951 0.199951)" />
                        </clipPath>
                      </defs>
                    </svg>
                  </div>
                </div>
                <div class="product-title">
                  <h4><?= $rc['name'] ?></h4>
                </div>
                <div class="product-price">
                  <p>&nbsp;<?= $rc['price'] ?></p>
                </div>
              </div>
            </a>
          <?php endforeach; ?>
        </div>

      </div>
      <!-- <button type="slick-button" class="slick-next">
        <i class="fa-solid fa-caret-right"></i>
      </button> -->
    </div>
  </div>
</section>

<section class="addcart">
  <div class="addcart-info">
    <!-- <div class="select-group">
      <div class="product-select">
        <p class="select-title">規格</p>
        <select class="select-info">
          <option>白色</option>
          <option>黑色</option>
          <option>咖啡色</option>
        </select>
      </div>
      <div class="product-select">
        <p class="select-title">尺寸</p>
        <select class="select-info">
          <option>S</option>
          <option>M</option>
          <option>L</option>
        </select>
      </div>
    </div> -->
    <div class="add-group">
      <div class="quantity d-flex ">
        <button class="quantity-reduce" onclick="numReduce()"><svg width="11" height="4" viewBox="0 0 18 4" fill="none" xmlns="http://www.w3.org/2000/svg">
            <rect width="18" height="4" rx="1" fill="#2d827d" />
          </svg>
        </button>
        <input class="quantity-num" value=1 id="mb_addtext"></input>
        <button class="quantity-add" onclick="numAdd()"><svg width="11" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path fill-rule="evenodd" clip-rule="evenodd" d="M7.2 17C7.2 17.5523 7.64772 18 8.2 18H9.8C10.3523 18 10.8 17.5523 10.8 17V10.8H17C17.5523 10.8 18 10.3523 18 9.8V8.2C18 7.64772 17.5523 7.2 17 7.2H10.8V1C10.8 0.447715 10.3523 0 9.8 0H8.2C7.64772 0 7.2 0.447716 7.2 1V7.2H1C0.447715 7.2 0 7.64772 0 8.2V9.8C0 10.3523 0.447716 10.8 1 10.8H7.2V17Z" fill="#ffffff" />
          </svg>
        </button>
      </div>
      <div class="cartbtn">
        <button class="add-cartbtn d-flex" data-sid="<?= $sid ?>" onclick="addToCartm(event)">
          <p>加入購物車</p>
          <div class="cart">
            <svg width="18" height="18" viewBox="0 0 26 23" fill="#ffffff" xmlns="http://www.w3.org/2000/svg">
              <path d="M4.88232 15.6203H20.3076V5.69214H1.72564C1.2217 5.69214 0.871237 6.1319 1.04519 6.54611L4.88232 15.6203Z" fill="" stroke="#ffffff" stroke-miterlimit="10" />
              <path d="M25.4566 0.546045L25.4564 0.545871L25.454 0.543485C25.454 0.543467 25.454 0.543448 25.454 0.543429C25.396 0.485505 25.3006 0.485524 25.2426 0.543485L19.9379 5.84555L20.1516 6.05907L25.4566 0.756655C25.5145 0.698812 25.5145 0.603888 25.4566 0.546045Z" fill="#ffffff" stroke="#ffffff" />
              <path d="M18.0269 20.701C18.0269 21.2252 18.4521 21.6507 18.9773 21.6507C19.5025 21.6507 19.9277 21.2252 19.9277 20.701C19.9277 20.1767 19.5024 19.7512 18.9773 19.7512C18.4522 19.7512 18.0269 20.1767 18.0269 20.701Z" fill="#ffffff" stroke="#ffffff" />
              <path d="M5.38233 20.701C5.38233 21.2252 5.80761 21.6507 6.33276 21.6507C6.85792 21.6507 7.2832 21.2252 7.2832 20.701C7.2832 20.1767 6.8579 19.7512 6.33276 19.7512C5.80762 19.7512 5.38233 20.1767 5.38233 20.701Z" fill="#ffffff" stroke="#ffffff" />
              <path d="M5.51918 17.9509H19.7908C19.8666 17.9509 19.9277 17.8895 19.9277 17.8143V17.8117C19.9277 17.7365 19.8666 17.675 19.7908 17.675H5.51918C5.44338 17.675 5.38221 17.7365 5.38221 17.8117V17.8143C5.38221 17.8907 5.44207 17.9509 5.51918 17.9509Z" fill="#ffffff" stroke="#ffffff" />
              <polyline points="15 13.5 17 15.5 22 10.5" stroke-width="1.5"></polyline>
            </svg>
          </div>
        </button>
      </div>
      <div class="heart-icon" data-sid="<?= $sid ?>" onclick="heart(event)">
        <svg width="20" height="20" viewBox="0 0 26 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path class="heart" d="M24.9375 6.30948L24.9317 6.25122L24.9192 6.19403C24.2754 3.2556 21.6922 1.0383 18.5737 1.0383C16.253 1.0383 14.2267 2.26615 13.0775 4.10766C11.9237 2.24427 9.87917 1 7.53714 1C3.92217 1 1.02082 3.96267 1.00002 7.58933H1V7.59507V7.59512V7.59518V7.59523V7.59528V7.59533V7.59539V7.59544V7.59549V7.59555V7.5956V7.59565V7.59571V7.59576V7.59581V7.59587V7.59592V7.59597V7.59603V7.59608V7.59614V7.59619V7.59624V7.5963V7.59635V7.59641V7.59646V7.59651V7.59657V7.59662V7.59668V7.59673V7.59678V7.59684V7.59689V7.59695V7.597V7.59706V7.59711V7.59717V7.59722V7.59728V7.59733V7.59738V7.59744V7.59749V7.59755V7.5976V7.59766V7.59771V7.59777V7.59783V7.59788V7.59794V7.59799V7.59805V7.5981V7.59816V7.59821V7.59827V7.59832V7.59838V7.59843V7.59849V7.59855V7.5986V7.59866V7.59871V7.59877V7.59882V7.59888V7.59894V7.59899V7.59905V7.5991V7.59916V7.59922V7.59927V7.59933V7.59938V7.59944V7.5995V7.59955V7.59961V7.59967V7.59972V7.59978V7.59984V7.59989V7.59995V7.60001V7.60006V7.60012V7.60018V7.60023V7.60029V7.60035V7.6004V7.60046V7.60052V7.60057V7.60063V7.60069V7.60074V7.6008V7.60086V7.60092V7.60097V7.60103V7.60109V7.60114V7.6012V7.60126V7.60132V7.60137V7.60143V7.60149V7.60155V7.6016V7.60166V7.60172V7.60178V7.60183V7.60189V7.60195V7.60201V7.60206V7.60212V7.60218V7.60224V7.60229V7.60235V7.60241V7.60247V7.60253V7.60258V7.60264V7.6027V7.60276V7.60282V7.60287V7.60293V7.60299V7.60305V7.60311V7.60316V7.60322V7.60328V7.60334V7.6034V7.60346V7.60351V7.60357V7.60363V7.60369V7.60375V7.60381V7.60386V7.60392V7.60398V7.60404V7.6041V7.60416V7.60421V7.60427V7.60433V7.60439V7.60445V7.60451V7.60457V7.60462V7.60468V7.60474V7.6048V7.60486V7.60492V7.60498V7.60504V7.60509V7.60515V7.60521V7.60527V7.60533V7.60539V7.60545V7.60551V7.60557V7.60562V7.60568V7.60574V7.6058V7.60586V7.60592V7.60598V7.60604V7.6061V7.60615V7.60621V7.60627V7.60633V7.60639V7.60645V7.60651V7.60657V7.60663V7.60669V7.60675V7.6068V7.60686V7.60692V7.60698V7.60704V7.6071V7.60716V7.60722V7.60728V7.60734V7.6074V7.60746V7.60752V7.60757V7.60763V7.60769V7.60775V7.60781V7.60787V7.60793V7.60799V7.60805V7.60811V7.60817V7.60823V7.60829V7.60835V7.60841V7.60846V7.60852V7.60858V7.60864V7.6087V7.60876V7.60882V7.60888V7.60894V7.609V7.60906V7.60912V7.60918V7.60924V7.6093V7.60935V7.60941V7.60947V7.60953V7.60959V7.60965V7.60971V7.60977V7.60983V7.60989V7.60995V7.61001V7.61007V7.61013V7.61019V7.61025V7.61031V7.61036V7.61042V7.61048V7.61054V7.6106V7.61066V7.61072V7.61078V7.61084V7.6109V7.61096V7.61102V7.61108V7.61114V7.6112V7.61126V7.61131V7.61137V7.61143V7.61149V7.61155V7.61161V7.61167V7.61173V7.61179V7.61185V7.61191V7.61197V7.61203V7.61208V7.61214V7.6122V7.61226V7.61232V7.61238V7.61244V7.6125V7.61256V7.61262V7.61268V7.61274V7.6128V7.61285V7.61291V7.61297V7.61303V7.61309V7.61315V7.61321V7.61327V7.61333V7.61339V7.61344V7.6135V7.61356V7.61362V7.61368V7.61374V7.6138V7.61386V7.61392V7.61398V7.61403V7.61409V7.61415V7.61421V7.61427V7.61433V7.61439V7.61445V7.6145V7.61456V7.61462V7.61468V7.61474V7.6148V7.61486V7.61492V7.61497V7.61503V7.61509V7.61515V7.61521V7.61527V7.61532V7.61538V7.61544V7.6155V7.61556V7.61562V7.61568V7.61573V7.61579V7.61585V7.61591V7.61597V7.61603V7.61608V7.61614V7.6162V7.61626V7.61632V7.61637V7.61643V7.61649V7.61655V7.61661V7.61666V7.61672V7.61678V7.61684V7.6169V7.61695V7.61701V7.61707V7.61713V7.61719V7.61724V7.6173V7.61736V7.61742V7.61747V7.61753V7.61759V7.61765V7.6177V7.61776V7.61782V7.61788V7.61793V7.61799V7.61805V7.61811V7.61816V7.61822V7.61828V7.61834V7.61839V7.61845V7.61851V7.61856V7.61862V7.61868V7.61874V7.61879V7.61885V7.61891V7.61896V7.61902V7.61908V7.61913V7.61919V7.61925V7.6193V7.61936V7.61942V7.61947V7.61953V7.61959V7.61964V7.6197V7.61976V7.61981V7.61987V7.61993V7.61998V7.62004V7.62009V7.62015V7.62021V7.62026V7.62032V7.62037V7.62043V7.62049V7.62054V7.6206V7.62065V7.62071V7.62077V7.62082V7.62088V7.62093V7.62099V7.62104V7.6211V7.62116V7.62121V7.62127V7.62132V7.62138V7.62143V7.62149V7.62154V7.6216V7.62165V7.62171V7.62176V7.62182V7.62187V7.62193V7.62198V7.62204V7.62209V7.62215V7.6222V7.62226V7.62231V7.62237V7.62242V7.62248V7.62253V7.62258V7.62264V7.62269V7.62275V7.6228V7.62286V7.62291V7.62296V7.62302V7.62307V7.62313V7.62318V7.62323V7.62329V7.62334V7.62339V7.62345V7.6235V7.62356V7.62361V7.62366V7.62372V7.62377V7.62382V7.62388V7.62393V7.62398V7.62404V7.62409V7.62414V7.62419V7.62425V7.6243V7.62435C1 8.39113 1.13087 9.12654 1.36639 9.81117C2.2291 12.6033 4.50686 15.4809 6.67792 17.7685C8.88167 20.0905 11.1006 21.9321 11.9848 22.6419L11.9867 22.6434C12.5747 23.113 13.4145 23.1239 14.0133 22.6456L14.0145 22.6446C14.8513 21.9741 16.8843 20.2885 18.9681 18.1415C21.0325 16.0144 23.2349 13.3462 24.3136 10.708L24.3136 10.708C24.7323 9.68393 25 8.62233 25 7.57255C25 7.1424 24.9783 6.72207 24.9375 6.30948Z" stroke="#d45a6a" stroke-width="2" />
        </svg>
      </div>
    </div>
  </div>
</section>

<div class="footer-pc">
  <div class="footer-pc-up">
    <div class="footer-nav">
      <ul>
        <li><a href="#">聯絡我們</a></li>
        <li><a href="./member-myIntro.php">會員中心</a></li>
      </ul>
    </div>
    <div class="footer-c">
      <p>&copy;&nbsp;bling&nbsp;bling&nbsp;gift,&nbsp;Inc.</p>
    </div>
    <div class="footer-social">
      <a href="#">
        <div class="send">
          <svg width="23" height="23" viewBox="0 0 23 23" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M11.1764 0C5.00142 0 0 5.00142 0 11.1764C0 17.3514 5.00142 22.3528 11.1764 22.3528C17.3514 22.3528 22.3528 17.3514 22.3528 11.1764C22.3528 5.00142 17.3514 0 11.1764 0ZM16.5033 7.26605C16.4712 7.71989 16.2145 9.30146 15.9532 11.0114C15.5635 13.4319 15.1464 16.0816 15.1464 16.0816C15.1464 16.0816 15.0822 16.8242 14.5321 16.9526C13.982 17.0809 13.0789 16.4987 12.9184 16.3704C12.7901 16.2741 10.4979 14.8209 9.65444 14.1103C9.42981 13.9178 9.16851 13.5281 9.68653 13.0789C10.8509 12.0153 12.24 10.6905 13.0789 9.84698C13.4685 9.45732 13.8536 8.55422 12.24 9.65444C9.94783 11.236 7.6878 12.7213 7.6878 12.7213C7.6878 12.7213 7.16978 13.0422 6.2025 12.7534C5.23522 12.4646 4.10291 12.0749 4.10291 12.0749C4.10291 12.0749 3.32817 11.589 4.65302 11.0756C4.65302 11.0756 10.2412 8.78343 12.1804 7.9766C12.923 7.6557 15.4444 6.61967 15.4444 6.61967C15.4444 6.61967 16.6088 6.16583 16.5079 7.26605H16.5033Z" fill="white" />
          </svg>
        </div>
      </a>
      <a href="#">
        <div class="facebook">
          <svg width="23" height="23" viewBox="0 0 23 23" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M11.8236 0C5.64864 0 0.647217 5.00142 0.647217 11.1764C0.647217 17.3514 5.64864 22.3528 11.8236 22.3528C17.9986 22.3528 23 17.3514 23 11.1764C23 5.00142 17.994 0 11.8236 0ZM16.2199 5.64322H14.9088C13.6207 5.64322 13.2172 6.44546 13.2172 7.26605V9.21435H16.0962L15.6377 12.217H13.2172V19.4693H9.97159V12.217H7.33565V9.21435H9.97159V6.92681C9.97159 4.32754 11.5211 2.88808 13.8911 2.88808C15.028 2.88808 16.2153 3.08979 16.2153 3.08979V5.64322H16.2199Z" fill="white" />
          </svg>
        </div>
      </a>
    </div>
  </div>
  <div class="footer-logo">
    <img src="./images/logo-footer.svg" alt="" />
  </div>
</div>

<?php include __DIR__ . '/parts/scripts.php'; ?>
<script src="./js/product-details.js"></script>
<?php include __DIR__ . '/parts/html-foot.php'; ?>